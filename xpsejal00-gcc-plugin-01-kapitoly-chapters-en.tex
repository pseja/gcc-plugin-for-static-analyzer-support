...
Developing correct software presents significant challenges. As software systems increase in size and complexity, certain categories of defects become undetectable through testing alone. Test suites can only evaluate explicitly covered execution paths, leaving much of the program's state space unexplored. Formal verification methods, particularly static program analysis, address this limitation by reasoning about program behavior without execution, thereby examining all reachable states or proving properties that hold unconditionally.
...
Despite its success, that being Predator winning gold medals at the international software verification competition SV-COMP and has been used to analyze real industrial code~\cite{CodeListener2012}, the original Code Listener architecture carries a number of design limitations that have accumulated as the tool suite evolved. Code is translated from GCC's GIMPLE intermediate representation into a stream of \emph{listener callbacks}, which is a sequence of events that first pass through a chain of filters, the most prominent being the \emph{switch-to-if} transformation that rewrites all \texttt{SWITCH} instructions into conditional branches, and are then accumulated by the \emph{code storage} component. Before an analyzer can inspect the resulting model, four analyses are always run unconditionally: a call graph is constructed, loop-closing edges are annotated, a points-to analysis is executed, and a liveness analysis marks dead variables. All of these transformations and analyses are invisible to the analyzer and cannot be disabled, even for clients that do not need them. The architecture also provides no standardized mechanism for an analyzer to attach its own annotations to the code model, no structured query interface, and no uniform support for exporting results in common formats.

The primary focus of this thesis is \emph{research and engineering}, specifically the design and implementation of a modernized reimplementation of Code Listener that addresses the aforementioned constraints. The new implementation, referred to throughout this text as the \emph{New Code Listener}, replaces the callback-driven pipeline with a persistent \emph{Code Model} that faithfully captures the GCC GIMPLE IR without requiring mandatory transformations. Analyses and transformations are demoted from hard-wired pre-analysis steps to optional, on-demand \emph{annotation services} that the analyzer requests explicitly. A~\emph{Query System} exposes the Code Model through a C++ view-based API (e.g., \texttt{codeModel.functions()} returns all function objects; filtering by identifier yields a single function). An \emph{Annotations Builder} allows analyzers to store arbitrary sidecar data keyed by entity identifier and annotation name. A~\emph{Diagnostic Reporter} lets analyzers emit warnings and errors in GCC's standard format. Finally, optional \emph{Exporters} can serialize the Code Model to JSON or a Graphviz visualization.
