 ...
 * @brief Implements translation from GCC trees and GIMPLE into the compiler-independent CodeModel.
 * @date 2026-05-02
 *
...
Core::TypeKind GCCAdapter::mapTypeTreeToTypeKind(tree &type_tree)
{
    if (!type_tree)
    {
        reporter.report(Core::DiagnosticLevel::Debug, "Encountered a null type tree, treating as UNKNOWN");
...
Core::SourceLocation GCCAdapter::getSourceLocation(location_t location)
{
    if (location == UNKNOWN_LOCATION || location <= BUILTINS_LOCATION)
    {
        reporter.report(Core::DiagnosticLevel::Debug, "Encountered an unknown or compiler-generated source location");
    ...
    {
        reporter.report(Core::DiagnosticLevel::Debug, "Encountered an unknown source location");

        return Core::SourceLocation();
    }

    Core::SourceLocation sloc(eloc.file, current_function_name, eloc.line, eloc.column);
    sloc.native_handle = reinterpret_cast<void *>(static_cast<uintptr_t>(location));
    return sloc;
}
...
Core::TypeId GCCAdapter::getOrCreateType(tree type_tree)
{
    if (!type_tree)
    {
        reporter.report(Core::DiagnosticLevel::Debug, "Encountered a type without a tree representation");
    ...
    case REAL_TYPE: {
        ...
        {
            reporter.report(Core::DiagnosticLevel::Debug, "Encountered a real type with BLKmode, which may indicate "
    ...
    default:
        reporter.report(Core::DiagnosticLevel::Debug, "Unhandled type tree code: " + type->name + "(" +
...
Core::VariableId GCCAdapter::getOrCreateVariable(tree variable_tree)
{
    if (!variable_tree)
    {
        reporter.report(Core::DiagnosticLevel::Debug, "Encountered a variable without a tree representation");
...
Core::Operand GCCAdapter::parseOperand(tree operand_tree)
{
    ...
    if (TREE_CODE(operand_tree) == CONST_DECL)
    {
        reporter.report(Core::DiagnosticLevel::Debug, "Resolving CONST_DECL operand through its initializer");

        tree value = DECL_INITIAL(operand_tree);
        if (value)
        {
            Core::Operand op = parseOperand(value);
            if (auto *constant = std::get_if<Core::ConstantOperand>(&op); constant && !constant->type_id.isValid())
            {
                constant->type_id = getOrCreateType(TREE_TYPE(operand_tree));
            }
...
void GCCAdapter::linkPredecessorsAndSuccessors(basic_block bb, Core::Block *block)
{
    ...
    FOR_EACH_EDGE(e, ei, bb->preds)
    {
        ...
        {
            reporter.report(Core::DiagnosticLevel::Debug, "Skipping predecessor edge with unresolved source block ID");
            continue;
        }
...
void GCCAdapter::processInstruction(gimple *stmt, Core::Block *block)
{
    ...
    case GIMPLE_COND: {
        ...
        cond.opcode = mapTreeCodeToOpCode(gimple_cond_code(cond_stmt));
        cond.lhs = parseOperand(gimple_cond_lhs(cond_stmt));
        cond.rhs = parseOperand(gimple_cond_rhs(cond_stmt));
    ...
    case GIMPLE_GOTO: {
        ...
        if (target_bb)
        {
            ...
            {
                reporter.report(Core::DiagnosticLevel::Debug, "Unresolved target block for GOTO instruction");
    ...
    case GIMPLE_ASSIGN: {
        ...
        enum gimple_rhs_class rhs_class = gimple_assign_rhs_class(assign_stmt);
        switch (rhs_class)
        {
        case GIMPLE_UNARY_RHS:
        case GIMPLE_SINGLE_RHS:
            assign.rhs1 = parseOperand(gimple_assign_rhs1(assign_stmt));
            break;
        case GIMPLE_BINARY_RHS:
            assign.rhs1 = parseOperand(gimple_assign_rhs1(assign_stmt));
            assign.rhs2 = parseOperand(gimple_assign_rhs2(assign_stmt));
            break;
        case GIMPLE_TERNARY_RHS:
            assign.rhs1 = parseOperand(gimple_assign_rhs1(assign_stmt));
            assign.rhs2 = parseOperand(gimple_assign_rhs2(assign_stmt));
            assign.rhs3 = parseOperand(gimple_assign_rhs3(assign_stmt));
            break;
    ...
    case GIMPLE_CALL: {
        ...
        {
            const char *builtin_name = DECL_NAME(fndecl) ? IDENTIFIER_POINTER(DECL_NAME(fndecl)) : nullptr;
            reporter.report(Core::DiagnosticLevel::Debug, "Processing a direct call to built-in function " +
                                                              std::string(builtin_name ? builtin_name : "<unnamed>"));
        }

        if (gimple_call_internal_p(call_stmt))
        {
            internal_fn ifn = gimple_call_internal_fn(call_stmt);
        ...
        {
            call.arguments.push_back(parseOperand(gimple_call_arg(call_stmt, i)));
        }
...
void GCCAdapter::processFunction(function *fun)
{
    ...
    {
        reporter.report(Core::DiagnosticLevel::Debug, "Adapter encountered a function without a name");
        return;
    }
