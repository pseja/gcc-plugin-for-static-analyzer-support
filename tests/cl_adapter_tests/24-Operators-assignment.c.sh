#!/usr/bin/env bash
# @file 24-Operators-assignment.c.sh
# @author Lukáš Pšeja <xpsejal00@vutbr.cz>
# @brief Tests compound assignment operators.
# @date 2026-04-15
#
...
jq -e '[.instructions[] | select(.kind == "ASSIGN" and .opcode == "SUB")] | length >= 1' "$JSON" >/dev/null
jq -e '[.instructions[] | select(.kind == "ASSIGN" and .opcode == "MUL")] | length >= 1' "$JSON" >/dev/null
jq -e '[.instructions[] | select(.kind == "ASSIGN" and .opcode == "DIV")] | length >= 1' "$JSON" >/dev/null
jq -e '[.instructions[] | select(.kind == "ASSIGN" and .opcode == "MOD")] | length >= 1' "$JSON" >/dev/null
